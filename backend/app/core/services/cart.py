...
from app.core.schemas.cart import CartItemCreate, CartItemUpdate, CartItem, CartResponse


class CartService:
    def __init__(self, session: AsyncSession, category_service, product_service,):
    ...
    def update_cart_item(self, cart_data: Dict[int, int], item: CartItemUpdate) -> Dict[int, int]:
        if item.product_id not in cart_data:
            raise HTTPException(status_code=404, detail=f"Product {item.product_id} not in cart")
        cart_data[item.product_id] = item.quantity
        return cart_data
    ...
    def get_cart_details(self, cart_data: Dict[int, int]) -> CartResponse:
        ...
        products = self.product_service.get_multiple_products_by_ids(product_ids)
